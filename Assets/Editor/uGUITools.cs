...
public class uGUITools : MonoBehaviour {
	...
	[MenuItem("uGUI/générer Map %q")]
	static void GenererMap(){
		foreach(Transform transform in Selection.transforms){
			Vector3 positionT=transform.position;
			Vector3 decalage=transform.GetComponent<SpriteRenderer>().sprite.bounds.size;
			for(int i=0;i<=15;i++)
			{
				for(int j=0;j<=30;j++)
				{
					positionT.y=transform.position.y-decalage.y*i;
					positionT.x=transform.position.x+decalage.x*j;
					Transform tnew=Transform.Instantiate(transform,positionT,Quaternion.identity) as Transform;
					tnew.name="case"+j+"-"+i;
					tnew.parent=transform.parent;
				}
			}
		}
	}
	/*[MenuItem("uGUI/repeter Sprite vers le bas %q")]
	static void RepeatSpriteBottom(){
		foreach(Transform transform in Selection.transforms){
			Vector3 positionT=transform.position;
			Vector3 decalage=transform.GetComponent<SpriteRenderer>().sprite.bounds.size;
			for(int i=1;i<15;i++)
			{
				positionT.y=positionT.y-decalage.y;
				Transform tnew=Transform.Instantiate(transform,positionT,Quaternion.identity) as Transform;
				tnew.name=transform.name+i;
				tnew.parent=transform.parent;
			}
		}
	}*/
	[MenuItem("uGUI/repeter Sprite vers la droite %g")]
	static void RepeatSpriteDroite(){
		foreach(Transform transform in Selection.transforms){
			Vector3 positionT=transform.position;
			Vector3 decalage=transform.GetComponent<SpriteRenderer>().sprite.bounds.size;
			for(int i=1;i<30;i++)
			{
				positionT.x=positionT.x+decalage.x;
				Transform tnew=Transform.Instantiate(transform,positionT,Quaternion.identity) as Transform;
				tnew.name=transform.name+i;
				tnew.parent=transform.parent;
			}
